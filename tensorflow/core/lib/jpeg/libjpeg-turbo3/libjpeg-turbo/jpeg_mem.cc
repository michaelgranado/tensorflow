...
//TODO
  ...
  if (use_cmyk) {
    ...
    tempdata = sandbox.malloc_in_sandbox<JSAMPLE>(cinfo.output_width * 4 * sizeof(JSAMPLE));
  } else if (need_realign_cropped_scanline) {
    // Temporary buffer used for MCU-aligned scanline data.
    tempdata = sandbox.malloc_in_sandbox<JSAMPLE>(cinfo.output_width * components * sizeof(JSAMPLE));
  }
