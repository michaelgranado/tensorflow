...
//  auto p_jerr = sandbox.malloc_in_sandbox<decoder_error_mgr>();
 ...
 // Set up standard JPEG error handling
  ...
  // Save Data
  if (width) *width = cinfo.output_width.unverified_safe_because("int value");
  if (height) *height = cinfo.output_height.unverified_safe_because("int value");
  if (components) *components = cinfo.output_components.unverified_safe_because("int value");
